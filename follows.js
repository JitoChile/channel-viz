(function ( $ ){
	...
	*/
	...
	var defaultKey		= 'kvXeeW8IvIKaWPiInmAdR3UFgKlSrCVGO6mx90mV72OeuFJF', // Unique master Xively API key to be used as a default
		defaultFeeds	= '1872553213', // Comma separated array of Xively Feed ID numbers
		applicationName	= 'Universidad de los Andes', // Replaces Xively logo in the header
		dataDuration	= '90days', // Default duration of data to be displayed // ref: https://xively.com/dev/docs/api/data/read/historical_data/
		dataInterval	= 10800, // Default interval for data to be displayed (in seconds)
		dataColor		= '0A1922', // CSS HEX value of color to represent data (omit leading #)
							...
							xively.datastream.history(feedId, datastream.id, {duration: duration, interval: interval, limit: 1000}, function(datastreamData) {
								...
								if(datastreamData.datapoints) {
									...
									//	points.push({x: new Date(datapoint.at).getTime()/1000.0, y: parseFloat(datapoint.value)});
									        points.push({x:parseFloat(datapoint.value) , y: parseFloat(datapoint.value)}); 
									});
			...
			xively.feed.history(id, {  duration: "6hours", interval: 30 }, function (data) {
				if(data.id == id) {
					...
					$('#feed-' + data.id + ' .duration-90').click(function() {
						$('#loadingData').foundation('reveal', 'open');
						updateFeeds(data.id, thisFeedDatastreams, '1year', 30000);
						return false;
					});
